...
namespace MobX.Serialization
{
    public class XOREncryptionAsset : EncryptionProviderAsset
    {
        ...
        public override async Task<byte[]> EncryptAsync(string content, string passPhrase)
        {
            return await Task.Run(() => Encrypt(content, passPhrase));
        }

        public override async Task<string> DecryptAsync(byte[] data, string passPhrase)
        {
